...
def test_templates_leastsq(load_example_data):
    ...
    for nproc in parallel_cores:
        ...
        np.testing.assert_almost_equal(gof, [  8.40916682,   41.41075389,   24.17508901,  14.13583212])
    for nproc in parallel_cores:
        gof=fit_templates(d, sampler='leastsq', use_redshift=True, nprocesses=nproc)
        np.testing.assert_almost_equal(gof, [ 4.18634996,  2.73534527,  2.77111264,  2.51829497])
...
#        print gof
        np.testing.assert_almost_equal(np.sort(gof), np.sort([ 2.4059210272, 2.0797560142, 1.7905944939, 3.57346633979]))
    for nproc in parallel_cores:
        gof=fit_templates(d, sampler='nested', use_redshift=True, nprocesses=nproc)
#        print gof
        np.testing.assert_almost_equal(np.sort(gof), np.sort([ 4.17113262208, 2.73807890918, 2.79493934974, 2.50869418371]))

def test_newling_leastsq(load_example_data):
    d=load_example_data
    for nproc in parallel_cores:
        gof=fit_parametric('newling', d, sampler='leastsq', nprocesses=nproc)
        np.testing.assert_almost_equal(gof, [ 0.83526717,  0.51772027,  1.1398396,   1.11812427])

def test_karpenka_leastsq(load_example_data):
    d=load_example_data
    for nproc in parallel_cores:
        gof=fit_parametric('karpenka', d, sampler='leastsq', nprocesses=nproc)
        np.testing.assert_almost_equal(gof, [ 0.82915774,  0.45095918,  1.00380726,  1.14341116])
...
def test_newling_nested(load_example_data):
    ...
    for nproc in parallel_cores:
        ...
        np.testing.assert_almost_equal(gof, [ 0.83587807,  0.52186865,  1.1369503,   1.12273183])
...
def test_karpenka_nested(load_example_data):
    ...
    for nproc in parallel_cores:
        ...
        np.testing.assert_almost_equal(gof, [ 0.8307367,   0.4544247,   1.04103566,  1.14468516])

def test_gp_extraction(load_example_data):
    d=load_example_data
    gof_truth={'gapp':[ 0.76875293,  0.4266906,   0.7617092,   0.8427292 ], 'george':[]}#TODO george truth
    for gpalgo in gpalgos:
        for nproc in parallel_cores:
            gof=fit_gp(d, gpalgo=gpalgo, nprocesses=nproc)
            np.testing.assert_almost_equal(gof, gof_truth[gpalgo])
