...
class NorollV20WFDAugment(GPAugment):
    """Augment the Wide-Fast-Deep (WFD) events in the LSST-like dataset
    corresponding to the `noroll_v2.0_10yrs.db` cadence. The
    augmentation uses Gaussian Process extrapolation method (see
    `snaugment.GPAugment`).
    """
    ...
    def __init__(self, dataset, path_saved_gps, objs_number_to_aug=None,
        ...
        **kwargs : dict, optional
        ...
        self._aug_method = 'GP augmentation; noroll_v2.0_10yrs WFD'
    ...
    def _choose_target_number_obs(self, aug_obj_metadata):
        ...
        gauss_choice = self._rs.choice(2, p=[0.728, 0.272])
        if gauss_choice == 0:
            mean = 67.04
            var = np.sqrt(115.31)
        elif gauss_choice == 1:
    ...
    def _compute_obs_uncertainty(self, aug_obj_data, aug_obj_metadata):
        ...
        # modeled with the logaritm of a Gaussian mixture model.
        # Lognormal parameters from GMM:
        pb_noises = {'lsstu': {'weights': np.array([0.73, 0.27]),
                               'means': np.array([1.96, 2.26]),
                               'covars': np.array([0.09, 0.43])},
                     'lsstg': {'weights': np.array([0.10, 0.90]),
                               'means': np.array([2.04, 1.28]),
                               'covars': np.array([0.98, 0.12])},
                     'lsstr': {'weights': np.array([0.68, 0.32]),
                               'means': np.array([1.58, 1.85]),
                               'covars': np.array([0.08, 0.29])},
                     'lssti': {'weights': np.array([0.63, 0.37]),
...
class PrestoV20WFDAugment(GPAugment):
    ...
    def _choose_target_number_obs(self, aug_obj_metadata):
        ...
        gauss_choice = self._rs.choice(3, p=[0.308, 0.218, 0.474])
        if gauss_choice == 0:
            mean = 29.84
            var = np.sqrt(126.43)
        elif gauss_choice == 1:
    ...
    def _compute_obs_uncertainty(self, aug_obj_data, aug_obj_metadata):
        ...
        # modeled with the logaritm of a Gaussian mixture model.
        # Lognormal parameters from GMM:
        pb_noises = {'lsstu': {'weights': np.array([0.63, 0.37]),
                               'means': np.array([1.98, 2.37]),
                               'covars': np.array([0.09, 0.30])},
                     'lsstg': {'weights': np.array([0.10, 0.90]),
                               'means': np.array([2.09, 1.40]),
                               'covars': np.array([0.91, 0.14])},
                     'lsstr': {'weights': np.array([0.22, 0.46, 0.32]),
                               'means': np.array([2.83, 1.58, 2.16]),
                               'covars': np.array([0.27, 0.08, 0.11])},
                     'lssti': {'weights': np.array([0.65, 0.35]),
                               'means': np.array([1.96, 2.46]),
                               'covars': np.array([0.09, 0.25])},
                     'lsstz': {'weights': np.array([1.]),
                               'means': np.array([2.69]),
                               'covars': np.array([0.17])},
                     'lssty': {'weights': np.array([0.63, 0.37]),
                               'means': np.array([3.08, 3.55]),
...
class BaselineV20SubsetWFDAugment(GPAugment):
    ...
    def _choose_target_number_obs(self, aug_obj_metadata):
        ...
        # Estimate the distribution of number of observations in WFD
        # with a mixture of 4 gaussian distributions.
        gauss_choice = self._rs.choice(4, p=[0.297, 0.440, 0.153, 0.110])
        if gauss_choice == 0:
            mean = 19.62
            var = np.sqrt(66.35)
        elif gauss_choice == 1:
    ...
    def _compute_obs_uncertainty(self, aug_obj_data, aug_obj_metadata):
        ...
        # modeled with the logaritm of a Gaussian mixture model.
        # Lognormal parameters from GMM:
        pb_noises = {'lsstu': {'weights': np.array([0.21, 0.79]),
                               'means': np.array([2.39, 1.94]),
                               'covars': np.array([0.51, 0.10])},
                     'lsstg': {'weights': np.array([0.89, 0.11]),
                               'means': np.array([1.27, 2.03]),
                               'covars': np.array([0.11, 0.91])},
                     'lsstr': {'weights': np.array([0.30, 0.70]),
                               'means': np.array([1.88, 1.56]),
                               'covars': np.array([0.32, 0.08])},
...
class NorollV20SubsetWFDAugment(GPAugment):
    ...
    def _choose_target_number_obs(self, aug_obj_metadata):
        ...
        # Estimate the distribution of number of observations in WFD
        # with a mixture of 3 gaussian distributions.
        gauss_choice = self._rs.choice(3, p=[0.694, 0.177, 0.129])
        if gauss_choice == 0:
            mean = 70.63
            var = np.sqrt(59.74)
        elif gauss_choice == 1:
    ...
    def _compute_obs_uncertainty(self, aug_obj_data, aug_obj_metadata):
        ...
        # modeled with the logaritm of a Gaussian mixture model.
        # Lognormal parameters from GMM:
        pb_noises = {'lsstu': {'weights': np.array([0.85, 0.15]),
                               'means': np.array([1.98, 2.42]),
                               'covars': np.array([0.11, 0.62])},
                     'lsstg': {'weights': np.array([0.10, 0.90]),
                               'means': np.array([2.10, 1.28]),
                               'covars': np.array([1.04, 0.12])},
                     'lsstr': {'weights': np.array([0.69, 0.31]),
                               'means': np.array([1.58, 1.86]),
                               'covars': np.array([0.08, 0.31])},
                     'lssti': {'weights': np.array([0.62, 0.38]),
                               'means': np.array([1.97, 2.44]),
                               'covars': np.array([0.08, 0.21])},
                     'lsstz': {'weights': np.array([1.]),
                               'means': np.array([2.67]),
                               'covars': np.array([0.14])},
                     'lssty': {'weights': np.array([1.]),
