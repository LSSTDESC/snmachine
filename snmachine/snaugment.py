...
class BaselineV20DDFAugment(GPAugment):
    """Augment the Deep Drilling Field (DDF) events in the LSST-like dataset
...
class BaselineV20WFDAugment(GPAugment):
    ...
    def _compute_obs_uncertainty(self, aug_obj_data, aug_obj_metadata):
        ...
        pb_noises = {'lsstu': (np.array([0.68, 0.32]),
                               np.array([1.91, 2.20]),
                               np.array([np.sqrt(0.09), np.sqrt(0.33)])),
                     'lsstg': (np.array([0.85, 0.15]),
                               np.array([1.24, 1.72]),
                               np.array([np.sqrt(0.10), np.sqrt(0.57)])),
                     'lsstr': (np.array([0.65, 0.35]),
                               np.array([1.55, 1.84]),
                               np.array([np.sqrt(0.08), np.sqrt(0.22)])),
                     'lssti': (np.array([0.37, 0.63]),
                               np.array([2.48, 1.97]),
                               np.array([np.sqrt(0.16), np.sqrt(0.08)])),
                     'lsstz': (np.array([1.]),
                               np.array([2.71]),
                               np.array([np.sqrt(0.13)])),
                     'lssty': (np.array([1.]),
                               np.array([3.27]),
                               np.array([np.sqrt(0.15)]))}
        ...
        for pb in aug_obj_data['filter']:  # go through each observation
            try:
                ...
                gauss_choice = self._rs.choice(a=len(params_pb[0]),
                                               p=params_pb[0])
...
class SniabciiDDFAugment(GPAugment):
    """Augment the Deep Drilling Field (DDF) events in the LSST-like dataset
    corresponding to the `baseline_nexp2_v1.7_10yrs.db` cadence. The
    augmentation uses Gaussian Process extrapolation method (see
    `snaugment.GPAugment`).
    """
    ...
    def __init__(self, dataset, path_saved_gps, objs_number_to_aug=None,
        ...
        This class augments the Deep Drilling Field (DDF) events in the
        LSST-like dataset corresponding to the `baseline_nexp2_v1.7_10yrs.db`
        cadence.
        ...
        **kwargs : dict, optional
        ...
        self._aug_method = 'GP augmentation; baseline_nexp2_v1.7_10yrs DDF'
    ...
    def _compute_obs_uncertainty(self, aug_obj_data, aug_obj_metadata):
        ...
        # modeled with the logaritm of a Gaussian mixture model.
        # Passband parameters
        pb_noises = {'lsstu': {'weights': [0.34556592, 0.57749715, 0.07693693],
                               'means': [0.77185, 1.19141415, 1.85652999],
                               'covars': [0.03299505, 0.03314455, 0.06379353]},
                     'lsstg': {'weights': [0.31367609, 0.14129098, 0.54503293],
                               'means': [-0.25116208, 0.6020499, 0.1060065],
                               'covars': [0.03202531, 0.06563274, 0.02546574]},
                     'lsstr': {'weights': [0.31646206, 0.17148511, 0.51205283],
                               'means': [-0.20202721, 0.46961199, 0.15050021],
                               'covars': [0.02614601, 0.1284051, 0.02230432]},
                     'lssti': {'weights': [0.30071046, 0.19382162, 0.50546791],
                               'means': [0.22949484, 0.88580653, 0.52422947],
                               'covars': [0.02129302, 0.13609355, 0.01712184]},
                     'lsstz': {'weights': [0.44948967, 0.33357324, 0.21693709],
                               'means': [0.92402643, 0.71355807, 1.25239874],
                               'covars': [0.01066763, 0.01201745, 0.03381948]},
                     'lssty': {'weights': [0.16064206, 0.34760465, 0.49175329],
