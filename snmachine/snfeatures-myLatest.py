...
def getGPChi2(iniTheta, kernel, y,x,err, gpTimes):
    ...
    if np.sum(np.diag(gpCov)<0) == 0:
        ...
        #print(gpCov)
        #print('')
    redChi2 = reducedChi2(y,x,err, gpObs)
    ...
    for fil in d.filter_set:
        if fil in filters:
            ...
            if gpalgo=='gapp':
                ...
                fluxObs   = y
                metric = np.sqrt(np.std(x)*2)
                gpObs, redChi2, gp = getGPChi2(iniTheta=np.array([400, metric, 2, 4, 4, 6, 6]), kernel='ExpSquared',
                ...
                if redChi2 > 2: # bad gp
                    ...
                    if redChi2_1 < 2: # good gp
                        gpObs, redChi2, gp = gpObs_1, redChi2_1, gp_1
                        ...
                        if redChi2_2 < 2: # good gp
                            ...
                            gpObs_3, redChi2_3, gp_3 = getGPChi2(iniTheta=np.array([10, 200, 2, 4, 340, 6, 6]),
                                                             kernel='ExpSquared+ExpSine2', y=y,x=x,err=err, gpTimes=gpTimes)
...
class WaveletFeatures(Features):
    ...
    def extract_features(self, d, initheta=[500, 20], save_output=None, output_root='features', nprocesses=24, restart='none', gpalgo='george', xmin=None, xmax=None):
        ...
        print('I am starting to extract the features!!!')
...
#        #Construct the covariance matrix
        ...
        C=np.mat(C)
        print('The Cov matrix is sparce '+str(scipy.sparse.issparse(C))+' (testing)')
    ...
    def extract_pca(self, object_names,  wavout):
        ...
        print('PCAfunc before initializing')
        vals, vec, mn=self.pca(wavout)
        mn=mn.flatten()
        print('PCAfunc after initializing')
        ...
        print('PCAfunc done best coeffs')
        eigs=vec[:, :ncomp]
        print('PCAfunc done eigenvects')
        comps=np.zeros([len(wavout), ncomp])

        print('PCAfunc before project_pca')
        for i in range(len(wavout)):
            coeffs=wavout[i]
            A=self.project_pca(coeffs-mn, eigs)
            comps[i]=A
        print('PCAfunc after project_pca')
