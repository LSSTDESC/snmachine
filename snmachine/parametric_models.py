...
class NewlingModel:
    ...
    def __init__(self, **kwargs):
        ...
        if 'limits' in kwargs:
            ...
            self.limits={'logA':[0, 10], 'phi':[-60, 100], 'logsigma':[-3, 4], 'logk':[-4, 4], 'logpsi':[-6, 10]}
            #self.limits={'logA':[0, 10], 'phi':[0, 100], 'logsigma':[1e-5, 4], 'logk':[1e-5, 4], 'psi':[0, 500]}
    ...
    def evaluate(self, t, params):
        ...
        Psi[t>tau]=psi
        Ft[t>phi] = A * (delta**k) * np.exp(-delta) * (k**(-k)) * np.exp(k)+Psi[t>phi]
        return Ft
...
class KarpenkaModel:
    ...
    def __init__(self, **kwargs):
        ...
        if 'limits' in kwargs:
            ...
            self.limits={'logA':[np.log(1e-5), np.log(1000)], 'logB':[np.log(1e-5), np.log(100)], 't0':[0, 100], 't1':[0, 100], 'T_rise':[0, 100], 'T_fall':[0, 100]}
            #self.limits={'logA':[1e-5, 1000], 'logB':[1e-5, 100], 't0':[0, 100], 't1':[0, 100], 'T_rise':[0, 100], 'T_fall':[0, 100]}
        if 'initial' in kwargs:
            self.initial=kwargs['initial'] #initial parameters for minuit search
        else:
